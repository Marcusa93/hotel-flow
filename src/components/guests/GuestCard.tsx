...
import { Mail, Calendar, ArrowUpRight, MessageCircle } from 'lucide-react';
...
export function GuestCard({ guest, bookingsCount, totalSpend, onClick, hotelName, onWhatsApp, onEmail }: GuestCardProps) {
    ...
    const handleEmail = (e: React.MouseEvent) => {
        ...
        if (onEmail) {
            ...
            window.open(`mailto:${guest.email}?subject=${encodeURIComponent(`${hotelName} - Información`)}`, '_blank');
        }
    };
        ...
        >
            ...
            >
                ...
                <div className="mt-auto grid grid-cols-2 gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                    <Button variant="outline" size="sm" className="w-full rounded-xl h-9 hover:bg-green-50 hover:text-green-600 hover:border-green-200" onClick={handleWhatsApp}>
                        <MessageCircle className="w-3.5 h-3.5 mr-2" />
                        WhatsApp
                    </Button>
                    <Button variant="outline" size="sm" className="w-full rounded-xl h-9 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200" onClick={handleEmail}>
